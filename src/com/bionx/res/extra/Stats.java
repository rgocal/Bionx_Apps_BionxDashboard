...
public class Stats implements Serializable, Comparator<Frequency> {
	...
	public Stats(List<Frequency> frequencies, Hashtable<String, Long> times,
			Long totalTime) {
	...
	public Double getPartialPercentage(Frequency freq) {
		if (previousStats == null) {
			throw new IllegalStateException(
					"setPreviousStats() hasn't been called yet");
		}
		long time = times.get(freq.getValue())
				- previousStats.times.get(freq.getValue());
	...
	public int compare(Frequency lhf, Frequency rhf) {
		if (sortMethod == SortMethod.Frequency) {
			...
			return getPartialPercentage(lhf).compareTo(
					getPartialPercentage(rhf));
		} else if (sortMethod == SortMethod.PartialPercentageDesc) {
			return -(getPartialPercentage(lhf)
					.compareTo(getPartialPercentage(rhf)));
		} else {
			/* Impossible to reach */
			return 0;
		}
	...
	public static int colorFromPercentage(float percentage) {
		float[] hsv = { (1 - percentage) * 120f, 0.9f, 0.9f, };
