...
public class WirelessManager extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		int[] left_views = { R.id.first_title, R.id.second_title,
				R.id.third_title, R.id.fourth_title, R.id.fifth_title };

		/*
		 * R.layout.info_layout is the same layout used for directory info.
		 * Re-using the layout for this activity, so id tag names may not make
		 * sense,but are in the correct order.
		 */
		int[] right_views = { R.id.dirs_label, R.id.files_label,
				R.id.time_stamp, R.id.total_size, R.id.free_space };
	...
	private void get_wifi() {
		...
		enable_label.setText(enabled ? "Your wifi is enabled"
				: "Your wifi is not enabled");
		...
		if (enabled) {
			data_labels[IPADD].setText(FileManager.integerToIPAddress(info
					.getIpAddress()));
