	...
	public void onStart(Intent intent, int startId) {
		...
		if (SysUtils.isRooted() && SysUtils.hasSysfs()) {
			...
			if (prefs.getBoolean(Constants.PREF_APPLY_ON_BOOT, false)) {
				String sound = prefs.getString(Constants.PREF_RINGTONE,
						Constants.PREF_DEFAULT_RINGTONE);
				String pattern = null;
				if (prefs.getBoolean(Constants.PREF_VIBRATE,
				 ...
				 */
				if (!prefs.getBoolean(Constants.CHECK_SHUTDOWN_OK, false)) {
					/*
					 * Phone didn't shut down properly.
					 */
					Log.i(Constants.APP_TAG, "detected bad shutdown");
					/*
					 * Check for uptime, sometimes it's just the UI layer which
					 * was killed by an OOM or by a (stupid) killall.
					 */
					long uptime = SystemClock.uptimeMillis();
					if (uptime > prefs.getLong(Constants.LAST_UPTIME, uptime)) {
						Log.i(Constants.APP_TAG,
								"detected a soft-reboot of the UI layer, skipping bad reboot notification");
					} else {
						/* Not a soft-reboot */
						SysUtils.showNotification(this,
								getString(R.string.notify_title_safety),
								getString(R.string.notify_safety_shutdown),
								sound, pattern);
					}
					 ...
					 */
					Log.i(Constants.APP_TAG, "detected NLJ's "
							+ Constants.NLJ_SAFETY_CHECK_DATA);
					SysUtils.showNotification(
							this,
							getString(R.string.notify_title_safety),
							getString(R.string.notify_nlj_safety_found,
									Constants.NLJ_SAFETY_CHECK_DATA), sound,
							pattern);
					 ...
					 */
					Log.i(Constants.APP_TAG, "detected NLJ's "
							+ Constants.NLJ_SAFETY_CHECK_EXT);
					SysUtils.showNotification(
							this,
							getString(R.string.notify_title_safety),
							getString(R.string.notify_nlj_safety_found,
									Constants.NLJ_SAFETY_CHECK_EXT), sound,
							pattern);
				} else {
					/*
					 * SG-1 you have a go!
					 */
					String min_freq = prefs.getString(Constants.PREF_MIN_FREQ,
							null);
					String max_freq = prefs.getString(Constants.PREF_MAX_FREQ,
							null);
					String governor = prefs.getString(Constants.PREF_GOVERNOR,
							null);
					String ioscheduler = prefs.getString(
							Constants.PREF_IOSCHEDULER, null);
					if (min_freq != null || max_freq != null
							|| governor != null || ioscheduler != null) {
						/*
						...
						boolean updated = SysUtils.setFrequenciesAndGovernor(
								min_freq, max_freq, governor, ioscheduler,
								this, R.string.ok_updated_freqs,
								R.string.err_update_failed);
						if (prefs.getBoolean(Constants.PREF_NOTIFY_ON_BOOT,
							...
							if (updated) {
								...
								SysUtils.showNotification(
										this,
										getString(R.string.notify_title_safety),
										getString(R.string.err_update_failed),
										sound, pattern);
							}
						}
					}
